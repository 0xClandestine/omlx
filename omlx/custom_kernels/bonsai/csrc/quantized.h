...
// T5_DQ4/T5_T1: SWAR-packed dequant tables (Identity I-D, pre-scaled per byte).
//
// Traditional SIMD-style "one load, many values": each constant load produces
// 4 (half4) or 1 (half) dequant value — the byte is converted to fp16 ONCE via
// the table index, and only the per-row scale multiply (one packed half4 mul
// per byte) remains. Values (trit-1) ∈ {-1,0,1} are exact fp16.
//
//   T5_DQ4[v] = (t0-1, t1-1, t2-1, t3-1)   T5_T1[v] = t4-1   (entries 243..255 unused)
constant constexpr half4 T5_DQ4[256] = {
    half4(-1.h, -1.h, -1.h, -1.h),
    half4(0.h, -1.h, -1.h, -1.h),
    half4(1.h, -1.h, -1.h, -1.h),
    half4(-1.h, 0.h, -1.h, -1.h),
    half4(0.h, 0.h, -1.h, -1.h),
    half4(1.h, 0.h, -1.h, -1.h),
    half4(-1.h, 1.h, -1.h, -1.h),
    half4(0.h, 1.h, -1.h, -1.h),
    half4(1.h, 1.h, -1.h, -1.h),
    half4(-1.h, -1.h, 0.h, -1.h),
    half4(0.h, -1.h, 0.h, -1.h),
    half4(1.h, -1.h, 0.h, -1.h),
    half4(-1.h, 0.h, 0.h, -1.h),
    ...
    half4(0.h, 1.h, 1.h, -1.h),
    half4(1.h, 1.h, 1.h, -1.h),
    half4(-1.h, -1.h, -1.h, 0.h),
    half4(0.h, -1.h, -1.h, 0.h),
    half4(1.h, -1.h, -1.h, 0.h),
    half4(-1.h, 0.h, -1.h, 0.h),
    half4(0.h, 0.h, -1.h, 0.h),
    half4(1.h, 0.h, -1.h, 0.h),
    half4(-1.h, 1.h, -1.h, 0.h),
    half4(0.h, 1.h, -1.h, 0.h),
    half4(1.h, 1.h, -1.h, 0.h),
    half4(-1.h, -1.h, 0.h, 0.h),
    half4(0.h, -1.h, 0.h, 0.h),
    half4(1.h, -1.h, 0.h, 0.h),
    half4(-1.h, 0.h, 0.h, 0.h),
    half4(0.h, 0.h, 0.h, 0.h),
    half4(1.h, 0.h, 0.h, 0.h),
    half4(-1.h, 1.h, 0.h, 0.h),
    half4(0.h, 1.h, 0.h, 0.h),
    half4(1.h, 1.h, 0.h, 0.h),
    ...
    half4(-1.h, -1.h, -1.h, -1.h),
    half4(-1.h, -1.h, -1.h, -1.h),};
constant constexpr half T5_T1[256] = {
    -1.h,
    -1.h,
    -1.h,
    -1.h,
...
// Unified t5 steel loader with two decode strategies (constexpr SWAR switch):
//   false → per-trit T5_TO_B4 extraction            (original loader)
//   true  → SWAR packed-fp16 tables (half4 per byte, one packed scale mul;
//           an 8-value chunk spans 2 bytes for t0≤2 and 3 for t0>2, demuxed
//           by register select — zero int↔fp conversions per trit).
template <typename T, short BROWS, short BCOLS, short dst_ld, short reduction_dim,
          short tgp_size, short group_size, short bpg, bool SWAR = false>
...
#pragma clang loop unroll(full)
  ...
  void swar_load_impl() const {
    // k_abs is group-aligned and the whole group is in bounds, so no partial
    // guard is needed: positions never reach the padding trits of the final
    // byte, and an 8-value chunk spans at most 3 bytes (t0>2) whose table
    // indexes stay inside the group's byte range (shown in the header).
    const int r0 = k_abs - g_abs * group_size + bj;  // first value, in-group
    const int bA = (r0 * 103) >> 9;                  // r0/5 (exact for r0 ≤ 854)
    const int t0 = r0 - 5 * bA;                      // r0%5 ∈ {0..4}
    const device uint8_t* wb_p = src + g_abs * bpg;
    T scale = *scales;
    // SWAR dequant: index the packed fp16 tables once per byte, scale with
    // one packed half4 multiply (values (trit-1) are exact fp16 → the result
    // is bit-identical to the reference's fp16 ws[] rounding; bf16 gains at
    // most one extra rounding, well inside the test tolerance).
    const half sh = half(float(scale));
    const half4 hA = half4(sh) * T5_DQ4[wb_p[bA]];
    const half4 hB = half4(sh) * T5_DQ4[wb_p[bA + 1]];
    const half4 hC = half4(sh) * T5_DQ4[wb_p[bA + 2]];
    const half   sA = sh * T5_T1[wb_p[bA]];
    const half   sB = sh * T5_T1[wb_p[bA + 1]];
    const half   sC = sh * T5_T1[wb_p[bA + 2]];
#pragma clang loop unroll(full)
    for (short j = 0; j < VPT; j++) {
      // value j = trit (t0+j) mod 5 of byte bA + (t0+j)/5
      const int toff = t0 + j;               // 0..12
      const int src  = (toff >= 10u) ? 2 : ((toff >= 5u) ? 1 : 0);
      const int tt   = toff - 5 * src;       // 0..4 within the source byte
      const half4 base = (src == 0u) ? hA : ((src == 1u) ? hB : hC);
      // component demux: tt 0..3 from the half4, tt==4 from the singletons
      const half cp = (tt == 4u) ? ((src == 0u) ? sA : ((src == 1u) ? sB : sC))
                    : (tt < 2u ? (tt < 1u ? base.x : base.y)
                               : (tt < 3u ? base.z : base.w));
      dst[j] = T(cp);
    }
...
METAL_FUNC void qmm_t5_steel_impl(
  ...
  using loader_w_t = T5BlockLoader<
      T, BN, BK, BK_padded, 1, WM * WN * SIMD_SIZE, group_size, bpg, SWAR>;
